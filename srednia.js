...
function avg() {
  chrome.storage.sync.get(
    ...
    (res) => {
      ...
      minus = res.minus;
      plus = res.plus;
      $(".container-background>.decorated.stretch>tbody>tr").each(parseRows);
    }
  );
}
...
avg();
function parseRows() {
  $(this).find("td:eq(3),td:eq(7)").addClass("center");
  if ($(this).attr("name") == undefined) {
    $(this).find("td:eq(2)").each(parseCell);
    if (wagi != 0 && srednia != 0)
      $(this)
        .find("td:eq(3)")
        .html(
    ...
    $(this).find("td:eq(6)").each(parseCell);
    if (wagi != 0 && srednia != 0)
      $(this)
        .find("td:eq(7)")
        .html(
...
function parseCell() {
  if ($(this).val() != "Brak ocen") $(this).children().each(parseGrade);
}
function parseGrade() {
...
function getGrade(grade) {
  if (grade.length == 1) {
    ...
    return (
      (grade[1] == "-" ? minus : grade[1] == "+" ? plus : 0) + parseInt(grade)
    );
  }
}
